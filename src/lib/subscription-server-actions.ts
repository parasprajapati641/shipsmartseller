...
export type ServerSubscriptionState = {
  plan: "trial" | "premium_plus" | "expired" | "unsubscribed";
...
type ServerStoreRecord = {
  subscription_plan: "trial" | "premium_plus" | "expired" | "unsubscribed";
...
/** Fetch server-validated subscription state (Auto-provisions 30-day free trial for new users) */
export async function fetchServerSubscriptionState(
  ...
  try {
    ...
    if (supabaseAdmin) {
      ...
      if (dbData) {
        const expiresAt = dbData.subscription_expires_at
          ? Number(dbData.subscription_expires_at)
          : dbData.expires_at
            ? Number(dbData.expires_at)
            : null;
        const isExpired = expiresAt ? now >= expiresAt : false;
        const isTrial = Boolean(dbData.is_trial ?? (dbData.subscription_plan === "trial"));

        record = {
          subscription_plan: isExpired
            ? "expired"
            : dbData.subscription_plan === "premium_plus"
              ? "premium_plus"
              : isTrial
                ? "trial"
                : "unsubscribed",
          ...
          subscription_started_at: dbData.subscription_started_at
            ...
            : dbData.started_at
              ...
              : now,
          subscription_expires_at: expiresAt ?? (now + thirtyDays),
  ...
  // Auto-provision 30-Day Free Trial if new user account
  if (!record) {
    record = {
      ...
      subscription_expires_at: now + thirtyDays,
      last_payment_id: null,
    };
    serverMemoryStore.set(normalized, record);

    // Save auto-provisioned 30-Day Trial to Supabase DB asynchronously
    try {
      const { supabaseAdmin } = await import("../integrations/supabase/client.server.js");
      if (supabaseAdmin) {
        await (
  ...
  // Check 30-Day Expiry: Current Time >= subscription_expires_at
  let plan = record.subscription_plan;
  let status = record.subscription_status;
  const expiresAt = record.subscription_expires_at;

  if (expiresAt && now >= expiresAt) {
    plan = "expired";
    status = "expired";
    record.subscription_plan = "expired";
    record.subscription_status = "expired";
    serverMemoryStore.set(normalized, record);
  }

  const isUnlimited = status === "active" && Boolean(expiresAt && now < expiresAt);
...
/** TanStack Server Function: Record successful generation for active user */
