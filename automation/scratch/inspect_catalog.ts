...
async function main() {
  ...
  if (await searchInput.isVisible({ timeout: 5_000 })) {
    ...
    const firstResult = page
      .locator('li, div[class*="result"], div[class*="item"], p:has-text("Sarees")')
      .first();
  ...
  const continueBtn = page
    .getByRole("button", { name: /add product|continue|next|select/i })
    .or(page.getByText(/add product|continue/i))
    .first();
  ...
  const uploadElements = await page.evaluate(() => {
    ...
    const clickables = Array.from(document.querySelectorAll("button, div, label, p, span"))
      ...
      .filter((el) =>
        /add front image|add primary image|upload front image|upload image|add image|drag and drop|select files/i.test(
          el.text || "",
        ),
      );
  ...
  await page.screenshot({
    path: "automation/.screenshots/final_image_upload_screen.png",
    fullPage: true,
  });
