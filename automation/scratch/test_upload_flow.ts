...
async function main() {
  ...
  if ((await fileInput.count().catch(() => 0)) > 0) {
    ...
    const dropzone = page
      .locator(
        'label[for="addImages"], div:has-text("Add Front Image"), div:has-text("Upload Image")',
      )
      .first();
  ...
  const thumbnail = page
    .locator('img[src*="blob:"], img[src*="meesho"], [class*="thumbnail"], [class*="preview"]')
    .first();
  ...
  const removeBtn = page
    ...
    .or(page.locator('[class*="delete"], [class*="remove"], svg[class*="trash"]'))
    .first();
  ...
  await page.screenshot({
    path: "automation/.screenshots/live_upload_success_verification.png",
    fullPage: true,
  });
